...
PARAMS = {
    ...
    # M3 corner bolt holes. Inset must clear the corner fillet: at
    # inset 6.0 with corner_radius 8.0 only ~3.5 mm of material was left
    # outside the bolt on the diagonal. 8.0 gives ~5.9 mm.
    'bolt_hole_dia':     3.4,
    'bolt_hole_inset':   8.0,
...
def _compute_pocket_polygons(p):
    ...
    # 3. Clearance: offset the polygon outward along true edge normals,
    # giving a uniform perpendicular gap on every edge.
    # Hole positions are unchanged — they're absolute landmarks.
    clear = p['pocket_clearance']
    inflated = _offset_polygon(norm_poly, clear)
...
def _signed_area(pts):
    """Shoelace area. Positive = counter-clockwise winding."""
    s = 0.0
    n = len(pts)
    for i in range(n):
        x0, y0 = pts[i]
...
def _offset_polygon(pts, delta, miter_limit=4.0):
    """Offset a simple polygon outward by delta along true edge normals.

    Each edge is shifted perpendicular to itself by delta, then adjacent
    shifted edges are intersected to give the new vertex. This yields a
    uniform perpendicular clearance on every edge.

    Do NOT replace this with radial expansion from the centroid — that
    only equals a true offset for a circle. On this outline it delivered
    as little as 0.40 mm of a requested 1.0 mm on the thumb-cluster
    diagonal, because those edges run nearly parallel to the centroid
    ray, so "outward" slid the vertex along the edge instead of away.

    Handles concave corners (this outline has five) and either winding.
    Sharp convex corners are mitered; the join is clamped to
    miter_limit * delta so a future outline edit with a near-zero corner
    cannot produce a runaway spike.
    """
    n = len(pts)
    if n < 3 or delta == 0.0:
        return list(pts)

    # Outward normal depends on winding: for CCW, edge (p0 -> p1) has
    ...
    for i in range(n):
        ...
        if length == 0.0:
            continue  # drop degenerate edge
        nx, ny = (dy / length) * sign, (-dx / length) * sign
        lines.append((
            (x0 + nx * delta, y0 + ny * delta),  # point on offset edge
            (dx / length, dy / length),          # unit direction
        ))
        # Original vertex this edge ends at — the corner the next join
        # replaces. Tracked explicitly because degenerate edges are
        # dropped, so indices into `lines` no longer match `pts`.
        corners.append((x1, y1))

    if len(lines) < 3:
        return list(pts)

    max_shift = abs(delta) * miter_limit
    m = len(lines)
    out = []
    for k in range(m):
        # Offset edge k runs from vertex k to vertex k+1, so the new
        # vertex following edge k is where offset edges k and k+1 meet.
        p_cur, d_cur = lines[k]
        p_next, d_next = lines[(k + 1) % m]
        corner = corners[k]  # original vertex this join replaces

        cross = d_cur[0] * d_next[1] - d_cur[1] * d_next[0]
        if abs(cross) < 1e-9:
            # Collinear continuation — the shifted endpoint is correct.
            out.append((p_next[0], p_next[1]))
            continue

        rx, ry = p_next[0] - p_cur[0], p_next[1] - p_cur[1]
        t = (rx * d_next[1] - ry * d_next[0]) / cross
        vx, vy = p_cur[0] + d_cur[0] * t, p_cur[1] + d_cur[1] * t

        # Miter limit applies to how far the join sits from the ORIGINAL
        # corner, not to t (which is a distance along the edge and is
        # naturally large on long edges). A sharp spike is clamped back
        # along the corner->join direction.
        shift = math.hypot(vx - corner[0], vy - corner[1])
        if shift > max_shift:
            scale = max_shift / shift
            vx = corner[0] + (vx - corner[0]) * scale
            vy = corner[1] + (vy - corner[1]) * scale
        out.append((vx, vy))

    # out[k] is the vertex AFTER edge k, i.e. original vertex k+1.
    # Rotate so the list starts at original vertex 0 again.
    return out[-1:] + out[:-1]
